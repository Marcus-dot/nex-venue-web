...
export default function LandingPage() {
  ...
  return (
    <div ref={containerRef} className="min-h-screen bg-background dark:bg-[#0f101e] overflow-hidden">
      ...
      <nav className="fixed top-0 left-0 w-full z-50 bg-background/50 dark:bg-[#0f101e]/80 backdrop-blur-xl border-b border-white/10 dark:border-white/5 px-8 py-4">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <Link href="/" className="flex items-center gap-2 group cursor-pointer">
            ...
            <span className="text-2xl font-bold tracking-tight text-surface-dark dark:text-white">NexVenue</span>
          </Link>

          <div className="hidden md:flex items-center gap-8">
            <Link href="#features" className="text-sm font-medium text-surface-dark/70 dark:text-white/60 hover:text-accent transition-colors">Features</Link>
      ...
      <main>
        ...
        <section ref={heroRef} className="relative pt-40 pb-32 px-8 min-h-screen flex items-center">
          <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            <div className="hero-content space-y-8">
              ...
              <p className="text-xl text-surface-dark/70 dark:text-white/60 font-normal max-w-lg leading-relaxed">
              ...
              <div className="flex flex-wrap gap-4">
                <Link href="/events">
                  <Button size="lg" className="px-8 text-lg group h-16">
                    Explore Events <ArrowRight className="ml-2 group-hover:translate-x-1 transition-transform" />
                  </Button>
                </Link>
                <Link href="/register">
                  <Button size="lg" variant="outline" className="px-8 text-lg h-16">
                    Host an Event
                  </Button>
                </Link>
              </div>
              ...
              <div className="flex flex-wrap gap-2 pt-4">
                {["Live agenda", "Q&A & polls", "Networking", "QR check-in"].map((cap) => (
                  <span key={cap} className="px-3 py-1.5 rounded-full text-xs font-medium bg-surface-dark/5 dark:bg-white/5 text-surface-dark/70 dark:text-white/60 border border-surface-dark/10 dark:border-white/10">
                    {cap}
                  </span>
                ))}
              </div>
            ...
            <div className="hidden lg:block relative">
              <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-3/4 h-3/4 bg-accent/10 blur-[100px] rounded-full" />
                ...
                <div className="space-y-3">
                  {[
                    ...
                    <div key={row.label} className="flex items-center gap-3 p-3 rounded-xl bg-surface-dark/5 dark:bg-white/5">
                      ...
                      <span className="text-sm font-medium text-surface-dark/80 dark:text-white/80">{row.label}</span>
                    </div>
                  ))}
                </div>
              </GlassCard>
          ...
          <div className="max-w-7xl mx-auto px-8 grid grid-cols-2 md:grid-cols-4 gap-12">
            {[
              ...
              <div key={i} className="stats-item text-center space-y-2">
                ...
                <div className="text-sm font-normal text-white/45">Built in</div>
              </div>
            ))}
          </div>
        </section>
        ...
        <section id="features" className="py-32 px-8">
          <div className="max-w-7xl mx-auto text-center mb-20">
            <h2 className="text-4xl md:text-5xl font-bold text-surface-dark dark:text-white tracking-tight mb-6">Built for modern events</h2>
            <p className="text-lg text-surface-dark/70 dark:text-white/60 font-normal max-w-2xl mx-auto">
          ...
          <div className="features-grid max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              ...
              <GlassCard key={i} className="feature-card p-10 group hover:border-accent/40 transition-colors">
                ...
                <h3 className="text-2xl font-bold text-surface-dark dark:text-white mb-4">{feature.title}</h3>
                <p className="text-surface-dark/70 dark:text-white/60 font-normal leading-relaxed">{feature.desc}</p>
              </GlassCard>
            ))}
          </div>
        </section>
      ...
      {/* Footer */}
      <footer className="bg-white/80 dark:bg-[#0f101e]/80 border-t border-surface-dark/5 dark:border-white/5 py-12 px-8">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-8">
          <Link href="/" className="flex items-center gap-2">
            <Image src="/nexvenue-logo.png" alt="NexVenue" width={32} height={32} className="rounded-lg" />
            <span className="text-xl font-bold tracking-tight text-surface-dark dark:text-white">NexVenue</span>
          </Link>
          <div className="flex gap-8">
            <Link href="/privacy" className="text-sm font-medium text-surface-dark/70 dark:text-white/60 hover:text-accent">Privacy</Link>
            <Link href="/terms" className="text-sm font-medium text-surface-dark/70 dark:text-white/60 hover:text-accent">Terms</Link>
            <a href="mailto:info@gralix.co" className="text-sm font-medium text-surface-dark/70 dark:text-white/60 hover:text-accent">Contact</a>
          </div>
          <div className="flex flex-col items-center md:items-end gap-2">
            <PoweredByGralix />
            <p className="text-sm font-normal text-surface-dark/55 dark:text-white/45">
              © 2026 <span className="text-surface-dark/70 dark:text-white/60">Gralix Technologies</span>
            </p>
          </div>
        </div>
      </footer>
